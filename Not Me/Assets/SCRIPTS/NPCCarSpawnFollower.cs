...
public class NPCCarSpawnFollower : MonoBehaviour
{
    ...
    {
        // Guard: don't call SetDestination until the agent is fully on the NavMesh.
        // This can happen for 1-2 frames after the agent is enabled — safe to skip.
    ...
    private void SetNPCVisible(bool visible)
    {
        ...
        if (visible)
        {
            // Enable the agent first, then warp it to the exact spawn position.
            // Warp() snaps the agent onto the NavMesh surface reliably —
            // without this, isOnNavMesh can stay false for several frames
            // and trigger the "not on NavMesh" error.
            _agent.enabled = true;
